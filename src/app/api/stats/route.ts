...
import { parseAllSessions, parseIntradayActivity, type IntradayActivityBucket } from "@/lib/transcriptParser";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const cycleStart = currentCycleStart(config.billingCycleStartDay);
    const cycleEnd = currentCycleEnd(config.billingCycleStartDay);
    const today = new Date();

    // ─── Cycle requests ────────────────────────────────────────────────────────
    const cycleSessions = sessions.filter(
      (s) => new Date(s.startedAt) >= cycleStart
    );
    const requestsThisCycle = cycleSessions.reduce(
      (sum, s) => sum + s.premiumRequests,
      0
    );
    const cycleUserTurns = cycleSessions.reduce((sum, s) => sum + s.userTurns, 0);
    const cycleAssistantTurns = cycleSessions.reduce((sum, s) => sum + s.assistantTurns, 0);
    const cycleToolCalls = cycleSessions.reduce((sum, s) => sum + s.toolCallsTotal, 0);
    const cycleDurationMinutes = cycleSessions.reduce((sum, s) => sum + s.durationMinutes, 0);
    ...
    const windowSessions = sessions.filter(
      (s) =>
        new Date(s.startedAt) >= avgWindowStart &&
        new Date(s.startedAt) <= today
    );
    const windowRequests = windowSessions.reduce(
      (sum, s) => sum + s.premiumRequests,
      0
    );
    ...
    const projectedExhaustionDate =
      daysLeft !== null
        ? toDateStr(addDays(today, daysLeft))
        : null;
    ...
    for (const dayStr of cycleDays) {
      ...
      if (isPast) {
        const bucket = bucketsMap[dayStr];
        cumulativeActual += bucket?.requests ?? 0;
    ...
    const skillMap: Record<
      string,
      { sessions: number; totalRequests: number; ratings: number[] }
    > = {};
    for (const s of sessions) {
      for (const skill of s.skillsActivated) {
        if (!skillMap[skill]) {
          skillMap[skill] = { sessions: 0, totalRequests: 0, ratings: [] };
        }
        skillMap[skill].sessions++;
        skillMap[skill].totalRequests += s.premiumRequests;
        const r = ratings[s.sessionId];
        if (r) skillMap[skill].ratings.push(r.quality);
      }
    }
    ...
    const skillStats: SkillStats[] = Object.entries(skillMap)
      .map(([name, data]) => ({
        ...
        qualityPer100Req:
          data.ratings.length > 0 && data.totalRequests > 0
            ? Math.round(
                ((data.ratings.reduce((a, b) => a + b, 0) / data.ratings.length) /
                  (data.totalRequests / data.sessions)) *
                  100 *
                  100
              ) / 100
            : null,
        liftVsBaseline:
          data.ratings.length > 0 && data.totalRequests > 0 && baselineQualityPer100Req !== null
            ? Math.round(
                ((((data.ratings.reduce((a, b) => a + b, 0) / data.ratings.length) /
                  (data.totalRequests / data.sessions)) *
                  100) -
                  baselineQualityPer100Req) *
                  100
              ) / 100
            : null,
      }))
      ...
      ratedSessions.length > 0
        ? Math.round(
            (ratedSessions.reduce(
              (sum, s) => sum + ratings[s.sessionId].quality,
              0
            ) /
